...
class _PortfoliosState extends State<Portfolios> {
  ...
  List<Map<String, dynamic>> portdata = List<Map<String, dynamic>>();
  ...
  getPortfolios(QuerySnapshot querySnapshot) {
    ...
    for (var doc in querySnapshot.docs) {
      data.add(doc.data());
      print(data[0]['portfolio']);
    }

    int i = 0;
    for (var stock in data[0]['portfolio']) {print("entered");
      if (portdata == null) {
        

        portdata[i]['portfolioName'] = stock['portfolioName'];
        portdata[i]['stocks'][i] = stock;
        i++;
