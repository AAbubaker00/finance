...
class QuoteObject {
  ...
  late String subIndustry = '';
  late String sector = '';

  late num regularMarketPrice = 0.0;
  late num regularMarketChange = 0.0;
  late num regularMarketChangePercent = 0.0;

  late num purchasePrice;
  late num quantity = 0.0;
  late num invested = 0.0;

  late num change = 0.0;
  late num changePercent = 0.0;
  late num value = 0.0;

  late bool dividendSupport;
  ...
  QuoteObject.fromMap(var document, {String? n, String? s, String? e})
      : name = n?? document.getElementsByClassName('PageTitleHOne').first.text.replaceRange(
        ...
        regularMarketPrice = double.parse(
  ...
  List<QuoteObject> combineToList(List<QuoteObject> a, List<QuoteObject> b) => List.generate(
      b.length,
      (index) =>
          combineTo(a.indexWhere((aExc) => aExc.symbol == b[index].symbol, orElse: () => null), b[index]));
