...
class _Calender extends State<Calender> {
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: CustomTextStyles(widget.dataObject.context)
                        .appBarTitleStyle.copyWith(fontWeight: FontWeight.w600),
                  ),
                  InkWell(
                    onTap: () {
                      selectedMonth =
  ...
  getDividends() async {
    ...
    for (var portfolio in widget.dataObject.portfolios) {
      for (var holding in portfolio.holdings) {
        if (holding.dividendSupport) {
          if (dividendHolding.firstWhere((h) => h.name == holding.name, orElse: () => null) == null) {
    ...
    if (dividendHolding.length > 0)
      widget.dataObject.dividends = await Marketbeat().getDividends( dividendHolding);
  }
  ...
  getEarning() async {
    ...
    for (var portfolio in widget.dataObject.portfolios) {
      for (var holding in portfolio.holdings) {
        ...
        if (exist == null) allHoldings.add(holding);
      }
    }
