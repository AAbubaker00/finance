  ...
  initalise() async{
    for (var portfolio in doc['portfolios']) {
      ...
      for (var holding in portfolio['holdings']) {
        QuoteObject hObject = await YahooApiService().getYahooQuote(symbol: holding['symbol'], exchange: holding['exchange']);
        hObject.quantity = holding['quantity']



        print(hObject.regularMarketPrice);
      }

      portfolios.add(PortfolioObject.fromMap(portfolio));
    }

    // print(portfolios.first.goal);
  }
