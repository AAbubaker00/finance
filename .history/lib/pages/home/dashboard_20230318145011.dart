...
class _DashboardState extends State<Dashboard> {
  ...
  getMarketQuotes() async {
    ...
    if (await Network('').getConnectionStatus()) {
      ...
      try {
        ...
        for (var pObject in widget.dataObject.portfolios) {
          ...
          for (var holding in pObject.holdings) {
            ...
            // PrintFunctions().printStartEndLine(holding.regularMarketPrice);
            // PrintFunctions().printStartEndLine(holding.purchasePrice);
      ...
      updateState = false;
      widget.dataObject.firstLoad = true;
  ...
  Future<bool> initalise() async {
    ...
    if (widget.dataObject.firstLoad == false ||
  ...
  void initState() {
    ...
    updateFrequency = Timer.periodic(Duration(seconds: 1), (timer) async {
