  ...
  getMarketQuotes() async {
    ...
    try {
      for (var pObject in widget.dataObject.portfolios) {
        ...
        List b = await YahooApiService().getYahooQuoteList(pObject.holdings);

        print(b);

        pObject.holdings.first.quantity;

        pObject.holdings = QuoteObject().combineToList(pObject.holdings, b);

        // print(pObject.holdings.first.quantity);
