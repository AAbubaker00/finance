import 'dart:async';
import 'dart:ui' as ui;
import 'package:Valuid/pages/community/social.dart';
import 'package:Valuid/pages/home/dashboard.dart';
import 'package:Valuid/pages/search/search.dart';
import 'package:Valuid/pages/calender/calender.dart';
import 'package:Valuid/services/Network/network.dart';
import 'package:Valuid/services/coingecko/coinGecko.dart';
import 'package:Valuid/services/database/database.dart';
import 'package:Valuid/services/robomarkets/roboMarkets.dart';
import 'package:Valuid/services/yahooapi/yahoo_api_provider.dart';
import 'package:Valuid/shared/custom_scaffold/custom_navigator.dart';
import 'package:Valuid/shared/dataObject/data_object.dart';
import 'package:Valuid/shared/dividends/dividends.dart';
import 'package:Valuid/shared/earnings/earnings.dart';
import 'package:Valuid/shared/files/fileHandler.dart';
import 'package:Valuid/shared/themes/themes.dart';
import 'package:Valuid/shared/update/initialise.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';
import 'package:intl/intl.dart';

class Home extends StatefulWidget {
  final Map data;

  const Home({Key key, @required this.data}) : super(key: key);

  @override
  _HomeState createState() => _HomeState();
}

class _HomeState extends State<Home> {

  @override
  void dispose() {
    super.dispose();
    timer.cancel();
  }

  final _auth = FirebaseAuth.instance;

  @override
  void initState() {
    super.initState();

    _marketPriceUpdateStreamController = new StreamController.broadcast();

    timer = Timer.periodic(Duration(seconds: 10), (timer) => _quickUpdate());

    loadData();
  }

  DateTime now = DateTime.parse(DateFormat('yyyy-MM-dd').format(DateTime.now()));

  Timer timer;

  StreamController<List> _marketPriceUpdateStreamController;

  bool isOnline = true, offlineAlert = false;

  DataObject dataObject;

  loadData() {
    dataObject = DataObject.fromMap(widget.data);
    dataObject.context = context;

    dataObject.userFire = _auth.currentUser;

    // if (dataObject.portfolios.length > 0 && dataObject.onPortfolio == null) {
    //   dataObject.onPortfolio = dataObject.portfolios.first;
    //   Initialise().updateOnHoldings(dataObject);
    // }
    // print(widget.data['notificationSettings']);
    // print(DateTime.parse(dataObject.lastCalenderUpdate).difference(DateTime.now()).inDays);

    // print(DateTime.fromMillisecondsSinceEpoch(1628146800 * 1000));

    // dataObject.isNotificationOn = dataObject.notificationsEvents.firstWhere(
    //             (notification) =>
    //                 (DateTime.parse(notification['date']).isBefore(now) ||
    //                     DateTime.parse(notification['date']).isAtSameMomentAs(now)) &&
    //                 notification['isSeen'] == false,
    //             orElse: () => false) ==
    //         false
    //     ? false
    //     : true;

    dataObject.ratio = (ui.window.physicalSize.height / ui.window.physicalSize.width);

    //`
    // dataObject.onHoldingTypes = [
    //   {'id': 'Stocks', 'items': dataObject.onStockHoldings},
    //   {'id': 'Cryptos', 'items': dataObject.onCryptoHoldings}
    // ];

    // PrintFunctions().printStartEndLine(ui.window.physicalSize.height);

    dataObject.userCurrencySymbol =
        Initialise(baseCurrency: dataObject.databaseData['profile_data']['baseCurrency'])
            .getCurrencySymbol()['symbol'];

    dataObject.userCurrency =
        Initialise(baseCurrency: dataObject.databaseData['profile_data']['baseCurrency'])
            .getCurrencySymbol()['short'];

    for (var dividend in widget.data['dividends']) {
      // print('hi');
      dataObject.dividends.add(Dividends.fromMap(dividend));

      // print(dataObject.dividends[0].currency);
    }

    for (var earning in widget.data['earnings']) {
      dataObject.earnings.add(Earnings.fromMap(earning));
    }

    for (var holiday in widget.data['holidays']) {
      dataObject.holidays.add(RoboMarketsHolidayObject.fromMap(holiday));
    }

    dataObject.theme = false;

    // PrintFunctions().printStartEndLine(dataObject.userCurrencySymbol);
    // PrintFunctions().printStartEndLine(dataObject.commodities);

    updatePrice(dataObject);
  }

  @override
  Widget build(BuildContext context) {
    dataObject.width = MediaQuery.of(context).size.width;
    dataObject.height = MediaQuery.of(context).size.height;

    // PrintFunctions().printStartEndLine(dataObject.height);

    // PrintFpdasunctions().printStartEndLine(CustomTextStyles(dataObject.theme, context).appBarTitleStyle.fontSize);

    return Container(
      color: UserThemes(dataObject.theme).backgroundColour,
      child: CustomScaffold(
        dataObject: dataObject,
        onItemTap: (index) {},
        children: [
          StreamBuilder(
              stream: _marketPriceUpdateStreamController.stream,
              builder: (context, AsyncSnapshot<dynamic> asyncSnapshot) => Dashboard(
                    dataObject: dataObject,
                  )),
          Calender(
            dataObject: dataObject,
          ),
          StreamProvider<QuerySnapshot>.value(
              initialData: null,
              value: DatabaseService().feed,
              builder: (context, snapshot) {
                return Community(
                  dataObject: dataObject,
                );
              }),
          // Search(
          //   dataObject: dataObject,
          // ),
          Search(
            dataObject: dataObject,
          ),
        ],
        scaffold: Scaffold(
          backgroundColor: UserThemes(dataObject.theme).backgroundColour,
          resizeToAvoidBottomInset: true,
          bottomNavigationBar: BottomNavigationBar(
              elevation: 0,
              iconSize: 1, // ADD THIS
              backgroundColor: Colors.transparent,
              selectedFontSize: 0, // ADD THIS
              unselectedFontSize: 0,
              showSelectedLabels: false,
              showUnselectedLabels: false,
              type: BottomNavigationBarType.fixed,
              selectedItemColor: UserThemes(dataObject.theme).blueVarient,
              unselectedItemColor: UserThemes(dataObject.theme).textColorVarient,
              onTap: (value) {},
              items: [
                BottomNavigationBarItem(label: 'Home', icon: Container()),
                BottomNavigationBarItem(icon: Container(), label: 'Calender'),
                BottomNavigationBarItem(icon: Container(), label: 'News'),
                BottomNavigationBarItem(icon: Container(), label: 'Search'),
              ]),
        ),
      ),
    );
  }

  getConnection() async {
    return await Network('').getConnectionStatus();
  }

  _quickUpdate() async {
    try {
      if ((DateTime.now().weekday != 6 && DateTime.now().weekday != 7) ||
          dataObject.lastCalenderUpdate == '') {
        int count = 0;

        isOnline = await getConnection();

        if (dataObject.offlineUpdate == true && isOnline == true) {
          dataObject.offlineUpdate = false;

          final _auth = FirebaseAuth.instance;
          User user = _auth.currentUser;

          await DatabaseService(uid: user.uid).updateChange(dataObject);
        }

        if (isOnline) {
          ([]..addAll(dataObject.stocks)..addAll(dataObject.cryptos)).forEach((holding) async {
            if (holding['typeId'] == 'stocks') {
              Map data = await YahooApiService()
                  .getYahooQuote(exchange: holding['exchange'], symbol: holding['symbol']);

              if (data != null && data.isNotEmpty && data != {}) {
                holding['marketData']['quote'] = data;
              }

              count++;
            } else if (holding['typeId'] == 'cryptos') {
              Map data = await CoinGeko().getCoinResult(holding['id']);

              // print(data);

              if (data != null && data.isNotEmpty && data != {}) {
                holding['marketData'] = data;
              } else {
                // print(holding['marketData']);
              }

              count++;
            } else if (holding['typeId'] == 'commodities') {
              dataObject.commodities = await DatabaseService().getCommodities();

              count++;
            }

            if (count == ([]..addAll(dataObject.stocks)..addAll(dataObject.cryptos)).length) {
              updatePrice(dataObject);

              _marketPriceUpdateStreamController.add([true]);
            }
          });
        } else {
          if (offlineAlert == true) {
          } else {
            // CustomeDialogs(dataObject.theme).noConnectionDialog(context);
            // setState(() {
            //   offlineAlert = true;
            // });
          }
        }
      }
    } catch (e) {
    }
  }
}

updatePrice(DataObject dataObject) {
  for (var portfolio in dataObject.portfolios) {
    for (var assetType in portfolio['assets']) {
      if (assetType['id'] == 'stocks') {
        for (var asset in assetType['items']) {
          if (dataObject.stocks.isEmpty) {
            dataObject.stocks.add(asset);
          } else {
            int isAssetExist =
                dataObject.stocks.indexWhere((element) => element['symbol'] == asset['symbol']);

            if (isAssetExist == -1) {
              dataObject.stocks.add(asset);
            }
          }
        }
      } else if (assetType['id'] == 'cryptos') {
        for (var asset in assetType['items']) {
          // asset['typeId'] = 'cryptos';

          if (dataObject.cryptos.isEmpty) {
            dataObject.cryptos.add(asset);
          } else {
            int isAssetExist =
                dataObject.cryptos.indexWhere((element) => element['symbol'] == asset['symbol']);

            if (isAssetExist == -1) {
              dataObject.cryptos.add(asset);
            }
          }
        }
      } else if (assetType['id'] == 'commodities') {
        for (var asset in assetType['items']) {
          // asset['typeId'] = 'cryptos';

          if (dataObject.commodities.isEmpty) {
            dataObject.commodity.add(asset);
          } else {
            int isAssetExist =
                dataObject.commodity.indexWhere((element) => element['symbol'] == asset['symbol']);

            if (isAssetExist == -1) {
              dataObject.commodity.add(asset);
            }
          }
        }
      }
    }
  }

  dataObject.allAccounts = {
    'invested': 0,
    'return': 0,
    'value': 0,
    'returnPercentage': 0,
  };

  for (var portfolio in dataObject.portfolios) {
    // print(dataObject.commodities);

    portfolio = Initialise(baseCurrency: dataObject.userCurrency, commodities: dataObject.commodities)
        .updatePortfolio(
      portfolio,
      rates: dataObject.rates,
      filteredHoldings: []..addAll(dataObject.stocks)..addAll(dataObject.cryptos),
    );

    dataObject.allAccounts['invested'] += portfolio['investedValue'];
    dataObject.allAccounts['value'] += portfolio['portfolioValue'];
    dataObject.allAccounts['return'] += portfolio['change'];
    dataObject.allAccounts['returnPercentage'] += portfolio['returnPercentage'];
  }

  dataObject.allAccounts['returnPercentage'] = dataObject.allAccounts['returnPercentage'].toStringAsFixed(2);
}
