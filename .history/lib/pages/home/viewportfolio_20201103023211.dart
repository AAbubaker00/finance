...
class _ViewPortfolioState extends State<ViewPortfolio>
                    ...
                    children: [
                      ...
                      Expanded(
                        ...
                        children: [
                          Container(
                              padding: EdgeInsets.only(top: 10),
                              child: SizedBox(child: SectorCharts())),
                          Container(
                            color: Colors.yellow,
                          ),
                          Container(
  ...
  sectorChart() {
    for (var stock in stocks) {
      double sInvestedValue = double.parse(stock['shares'].toString()) *
      ...
      if (sectors.isEmpty) {
        sectors.add({
          ...
          'vaue': sInvestedValue,
          'stocks':  stockFilter(sectorStocks, stock)
        });
      } else {
  ...
  List<Map<dynamic, dynamic>> stockFilter(
      List<Map<dynamic, dynamic>> stocks, dynamic stock) {
    List<Map<dynamic, dynamic>> filteredStocks = [];

    for (var s in stocks) {
      if (s['marketData']['assets']['sector'] == portfolioName) {
