...
class _ViewPortfolioState extends State<ViewPortfolio>
  ...
  _report() {
    return Column(
      children: [
        ...
        // Expanded(
        //     child: SizedBox(
        //         height: 50, child: SelectionLineHighlight.withSampleData())),
        Container(
  ...
  _dividends() {
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        //   child: ListView(
        //       shrinkWrap: true,
        //       children: (months.firstWhere(
        //               (month) => month['status'] == 'active' && months.indexOf(upComingDividends) < months.indexOf(month), orElse: () => {'stocks':[]}))['stocks'].map<Widget>((stockk) {
        //         return (stockk.isEmpty)? Center(child: Text('No Scheduled Dividends ')): Text("sdada");
        //       }).toList()),
        // )
      ],
    );
          ...
          child: Column(
            ...
            children: [
              ...
              ConstrainedBox(
                ...
                child: SizedBox(child: LineChartSample2()),
              )
            ],
          ),
        ),
  ...
  setCAGRChart() {
    ...
    for (var plot in cagrData) {
      cagrDataTable.add(FlSpot(index, double.parse(plot['value'].toString())));
      index++;

      interval = (interval == 0.0)? double.parse(interval.toStringAsFixed(2)): ((interval > double.parse(plot['value'].toString()))? interval: plot['value'].toString());
    }
