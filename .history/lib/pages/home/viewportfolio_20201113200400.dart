...
/* -------------------------------------------------------------------------- */
  ...
  stockChart(Map stock) {
    List<FlSpot> histroicalData = [];
    double index = 0.0;

    for (var plot in stock['marketData']['historicalData']["values"]) {
      histroicalData.add(FlSpot(index, double.parse(plot.toString())));
      index++;
    }

    print(histroicalData);
  ...
  List<Color> gradientColors = [Colors.green, Colors.transparent];
