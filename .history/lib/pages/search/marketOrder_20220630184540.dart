...
class MarketOrder extends StatefulWidget {
  ...
  final Map widget.instrument;

  MarketOrder({Key key, @required this.dataObject, @required this.widget.instrument}) : super(key: key);
...
class _MarketOrderState extends State<MarketOrder> {
  ...
  void initState() {
    super.initState();
    currencySymbol =
        Initialise(baseCurrency: widget.widget.instrument['quote']['currency']).getCurrencySymbol()['symbol'];

    instrumentSymbol = widget.widget.instrument['quote']['symbol'];
    instrumentExchange = widget.widget.instrument['quote']['exchange'];
    instrumentName = widget.widget.instrument['quote']['shortName'];
  }
                        ...
                        children: [
                          Text(
                            instrumentExchange == 'LSE'
                                ? '${(widget.widget.instrument['quote']['regularMarketPrice'] / 100).toStringAsFixed(0).replaceAllMapped(new RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}'
                      ...
                      if (portfolio['name'] == selectedPortfolio) {
                        ...
                        if (isHoldingExist == null) {
                          portfolio['assets'][0]['items'].add({
                            ...
                            'marketData': widget.widget.instrument
                          });
