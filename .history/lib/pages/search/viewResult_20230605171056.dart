...
class ViewResult extends StatefulWidget {
  ...
  const ViewResult({Key? key,required this.dataObject, this.result}) : super(key: key);
  @override
...
class _ViewResult extends State<ViewResult> {
  ...
  late QuoteObject quoteResult;

  Future<bool> getQuote() async {
    quoteResult = await Marketbeat()
        .getMarketbeatQuote(widget.result);
                      ...
                      function: () async {
                        if (_formKey.currentState!.validate() && widget.dataObject.portfolios.length > 0) {
                          ...
                          var duplicateCheck = widget.dataObject.onPortfolio!.holdings
                              .indexWhere((holding) => holding.name == quoteResult.name);

                          if (duplicateCheck == -1) {
                            widget.dataObject.onPortfolio.holdings.add(quoteResult);
                          } else {
                            widget.dataObject.onPortfolio.holdings.remove( widget.dataObject.onPortfolio.holdings[duplicateCheck]);
                            widget.dataObject.onPortfolio.holdings.add(quoteResult);
                          }
