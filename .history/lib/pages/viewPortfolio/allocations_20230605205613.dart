...
class Allocations extends StatefulWidget {
  ...
  Allocations({Key? key, required this.dataObject}) : super(key: key);
...
class _AllocationsState extends State<Allocations> {
  ...
  setDiversificationData() async {
    ...
    switch (diversificationOption) {
      case 'Dividends':
        for (var holding in widget.dataObject.onPortfolio!.holdings) {
          if (holding.dividendSupport) {
            var isObjectExist = diversificationCDT.indexWhere((type) => type.name == 'Dividends');
            ...
            if (isObjectExist == -1) {
              ...
              var obj = diversificationCDT[isObjectExist];
              diversificationCDT.remove(diversificationCDT[isObjectExist]);

              diversificationCDT.add(GeneralObject(
                  name: obj.name,
                  value: obj.value! + holding.invested,
                  assetList: obj.assetList! + [holding],
                  weight: obj.weight! +
                      (holding.invested / widget.dataObject.onPortfolio!.invested) * 100));
            }
          } else {
            var isObjectExist = diversificationCDT.indexWhere((type) => type.name == 'Non-Dividends');
            ...
            if (isObjectExist == -1) {
              ...
              diversificationCDT.add(GeneralObject(
                  name: diversificationCDT[isObjectExist].name,
                  value: diversificationCDT[isObjectExist].value! + holding.invested,
                  assetList: diversificationCDT[isObjectExist].assetList! + [holding],
                  weight: diversificationCDT[isObjectExist].weight! +
                      (holding.invested / widget.dataObject.onPortfolio!.invested) * 100));
            }
          }
        }
      ...
      case 'Exchanges':
        for (var holding in widget.dataObject.onPortfolio!.holdings) {
          var isGeneralObjectExist = diversificationCDT
              .indexWhere((region) => region.name == holding.exchange.toString().capitalizeAll());

          if (isGeneralObjectExist == -1) {
            diversificationCDT.add(GeneralObject(
            ...
            diversificationCDT.remove(diversificationCDT[isGeneralObjectExist]);

            diversificationCDT.add(GeneralObject(
                name: diversificationCDT[isGeneralObjectExist].name,
                value: diversificationCDT[isGeneralObjectExist].value! + holding.invested,
                assetList: diversificationCDT[isGeneralObjectExist].assetList! + [holding],
                weight: diversificationCDT[isGeneralObjectExist].weight! +
                    (holding.invested / widget.dataObject.onPortfolio!.invested) * 100));
          }
        }
        break;
      case 'Sectors':
        for (var holding in widget.dataObject.onPortfolio!.holdings) {
          var isGeneralObjectExist = diversificationCDT.indexWhere((region) => region.name == holding.sector);
      ...
      case 'Industries':
        for (var holding in widget.dataObject.onPortfolio!.holdings) {
          var isGeneralObjectExist =
              diversificationCDT.indexWhere((region) => region.name == holding.industry);
      ...
      case 'Currencies':
        for (var holding in widget.dataObject.onPortfolio!.holdings) {
          var isGeneralObjectExist =
              diversificationCDT.indexWhere((region) => region.name == holding.currency);
