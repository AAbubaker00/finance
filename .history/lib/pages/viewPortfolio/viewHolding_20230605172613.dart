...
class ViewHolding extends StatefulWidget {
  ...
  const ViewHolding({Key? key,required this.dataObject}) : super(key: key);
  @override
...
class _ViewHolding extends State<ViewHolding> {
  ...
  void initState() {
    ...
    hintText_0 = widget.dataObject.onHolding.purchasePrice.toStringAsFixed(2);
    hintText_1 = widget.dataObject.onHolding.quantity.toStringAsFixed(2);
  }
  ...
  Widget build(BuildContext context) {
      ...
      showModalBottomSheet(
          ...
          builder: (ctxt) {
            return CWConfirmBottomSheetButton(
              ...
              btnText:
                  'Are you sure you want to remove ${widget.dataObject.onHolding.name} from ${widget.dataObject.onPortfolio!.name}?',
              function: () async {
                widget.dataObject.onPortfolio!.holdings.remove(widget.dataObject.onHolding);
                ...
                Navigator.push(
                    ...
                    CustomPageRouteSlideTransition(
                        ...
                        child: RemovalCompletedNotification(
                          name: widget.dataObject.onHolding.name,
                          isStock: true,
                          secondaryName: widget.dataObject.onPortfolio!.name,
                        )));
              },
            );
          });
    ...
    return CWScaffold(
      ...
      body: Form(
        key: _formKey,
        child: CWListView(
          padding: const EdgeInsets.only(bottom: 20, top: 30, left: 15, right: 15),
            ...
            Column(
              children: [
                ...
                Column(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  ...
                  Text(
                    '${widget.dataObject.onHolding.currency} ${widget.dataObject.onHolding.change.toStringAsFixed(2).replaceAllMapped(new RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}',
                    style: CustomTextStyles(widget.dataObject.context).portfolioNameStyle.copyWith(
                        fontWeight: FontWeight.w600,
                        color: widget.dataObject.onHolding.change >= 0 ? greenVarient : redVarient),
                  ),
                ]),
              ],
            ),
              ...
              child: CWApplyButton(
                ...
                function: () async {
                  if (purchasePrice != 0 || quantity != 0) {
                    if (purchasePrice > 0) {
                      widget.dataObject.onHolding.purchasePrice = purchasePrice;
                    }

                    if (quantity > 0) {
                      widget.dataObject.onHolding.quantity = quantity;
                    }
