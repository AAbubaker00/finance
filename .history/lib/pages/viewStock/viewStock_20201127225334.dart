...
class _ViewStockState extends State<ViewStock> {
  ...
  _getDefaultLineChart() {
    ...
    List<_ChartData> chartData = List<_ChartData>();
    double dateIndex = 0.0;

    for (double plot in selectedStock['chartData']['max']['close']) {
      // if (plot == 'null') {
      //   // plot = plot.toString().replaceAll('null', '${prevValue.toString()}');
      //   // chartData.add(_ChartData(double.parse(plot.toString()), dateIndex));
      //   // dateIndex++;
      //   print('yes');
      // } else {
      chartData.add(_ChartData(plot, dateIndex));
      // }
    ...
    return SfCartesianChart(
      ...
      series: <ChartSeries<_ChartData, dynamic>>[
        HiloOpenCloseSeries(
            ...
            lowValueMapper: (_ChartData dateIndex, _) => dateIndex.dateIndex,
            highValueMapper: (_ChartData dateIndex, _) => dateIndex.dateIndex,
            openValueMapper: (_ChartData dateIndex, _) => dateIndex.dateIndex,
            closeValueMapper: null(_ChartData dateIndex, _) => dateIndex.dateIndex)
