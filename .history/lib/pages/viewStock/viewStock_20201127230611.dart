...
class _ViewStockState extends State<ViewStock> {
  ...
  _getDefaultLineChart() {
    print(selectedStock['chartDataTable']['max']['close']);

    List<_ChartData> chartDataTable = List<_ChartData>();
    double dateIndex = 0.0;

    for (var plot in selectedStock['chartDataTable']['max']) {
      // if (plot == 'null') {
      ...
      chartDataTable.add(_ChartData(open: plot['open'][dateIndex], close: plot['close'][dateIndex], high: plot['high'][dateIndex], low: plot['open'][dateIndex]));
