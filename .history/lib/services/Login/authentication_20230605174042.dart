...
class AuthService extends ChangeNotifier {
  ...
  UserObject? _userFromFirebaseUser(User? user) {
    try {
      if (user != null) {
        return UserObject(uid: user.uid, email: user.email.isNull? 'n/a' : user.email, name: '');
      } else {
        return null;
      }
    } catch (e) {
  ...
  Future registerWithEmailAndPassword({required String email, String password}) async {
    try {
