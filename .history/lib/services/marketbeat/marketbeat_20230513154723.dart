...
//Getting the response from the targeted url
  ...
  Future<List<Dividends>> getQuote() async {
//Getting the response from the targeted url

    final response = await http.Client().get(Uri.parse('https://www.marketbeat.com/stocks/NASDAQ/AAPL/'));

    if (response.statusCode == 200) {
      var document = parser.parse(response.body);

      // debugPrint(document.getElementsByClassName('price')[0].children.first.text.replaceRange(
      //     1, document.getElementsByClassName('price')[0].children.first.text.length, '')); //symbol
      // debugPrint(ForexConversion().currencySymbols.firstWhere(
      //     (currency) =>
      //         currency['symbol'] ==
      ...
      //     orElse: () => null)['short']); //currency

      // debugPrint(document.getElementsByClassName('price')[0].children.first.text.replaceRange(0, 1, '')); //regularmarketPrice
      debugPrint((document.getElementsByClassName('price')[0].children[1].text.replaceRange(0, 1, '').split(' ').first.replaceRange(document.getElementsByClassName('price')[0].children[1].text.replaceRange(0, 1, '').split(' ').first.indexOf('('), document.getElementsByClassName('price')[0].children[1].text.replaceRange(0, 1, '').split(' ').first.indexOf(pattern), replacement) ).toString()); //regularmarketPrice
      ...
      // debugPrint(document.getElementsByClassName('m-0 p-0')[1].children.first.children[1].text); //exchange
      // debugPrint(document.getElementsByClassName('m-0 p-0')[1].children[1].children[1].text); //industry
      // debugPrint(document.getElementsByClassName('m-0 p-0')[1].children[2].children[1].text); //Sub-Industry
      // debugPrint(document.getElementsByClassName('m-0 p-0')[1].children[3].children[1].text); //Sub-Industry

      // for (var section in document.getElementsByClassName('m-0 p-0')) {

      //   for(var element in section.children){
      //     print(object)
      //   }
      // }
    }
  ...
  exchangeCheck(QuoteObject holding) {
    switch (holding.fullExchangeName) {
      case 'NasdaqGS':
        exchange = 'NASDAQ';
        break;
      case 'LSE':
        ...
        break;
      default:
        exchange = holding.fullExchangeName;
        break;
    }
  }
}
