...
//Getting the response from the targeted url
  ...
  Future<QuoteObject> getQuote(Map result) async {
    ...
    final response =
        await http.Client().get(Uri.parse('https://www.marketbeat.com/stocks/$exchange/$symbol/'));

    if (response.statusCode == 200) {
      var document = parser.parse(response.body);

      print(document.getElementsByClassName('price')[0].children.first.text.replaceRange(0, 1, ''));
  ...
  preCheck(Map result) {
    switch (result['exchDisp']) {
      ...
      case 'LSE':
        exchange = 'LON';
        symbol = result['symbol'].toString().replaceAll('.L', '');
        break;
      default:
