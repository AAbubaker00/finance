  ...
  Future<QuoteObject> getMarketbeatQuote(Map result) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(document.getElementsByClassName('PageTitleHOne').first.text.replaceRange(document.getElementsByClassName('PageTitleHOne').first.text.indexOf('('), document.getElementsByClassName('PageTitleHOne').first.text.length, replacement));
    ...
    try {
      ...
      List<QuoteObject> responseJson = List.generate(
          ...
          (index) => QuoteObject.fromMap(parser.parse(response[index].body),
              s: holdings[index].symbol, e: holdings[index].exchange, n: holdings[index].name));
