...
class Marketbeat {
  ...
  Future<List<Dividends>> getDividends({required List<QuoteObject> holdings}) async {
//Getting the response from the targeted url
  ...
  Future<List<QuoteObject>> getMarketbeatQuoteList(List<QuoteObject> holdings) async {
    try {
      ...
      List responses = await Future.wait(resposeFutures as Iterable<Future<http.Response>>);

      sp.stop();
      print(sp.elapsedMilliseconds);
      sp.reset();
      sp.start();
      print(parser.parse(responses.first.body).body);

      List<QuoteObject> responseJson = [];

      for (var rep in responses) {
       responseJson.add(value) QuoteObject.fromMap(parser.parse(rep.body),
            s: holdings[responses.indexOf(rep)].symbol, e: holdings[responses.indexOf(rep)].exchange);
      }
