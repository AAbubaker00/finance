...
class Yahoo {
  static Future<Map<String, num>> fx(
      String base, List<String> quotes, http.Client client, Logger logger) async {
    final Map<String, num> results = <String, num>{};

    try {
      final Map<String, dynamic> fxQuotes = await _getFxQuotes(base, quotes, client, logger);
  ...
  static Future<Map<String, dynamic>> _getFxQuotes(
      String base, List<String> quotes, http.Client client, Logger logger) async {
    final List<String> symbolList = quotes.map((String quote) => base + quote + '=X').toList();

    return await FinanceQuote.getRawData(
        quoteProvider: QuoteProvider.yahoo, symbols: symbolList, client: client, logger: logger);
  }
}
