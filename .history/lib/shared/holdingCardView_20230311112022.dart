              ...
              children: [
                Text(
                    holding.quantity > 10000000
                        ? '${NumberFormat.compact().format(holding.quantity)} units '
                        : '${holding.quantity.toStringAsFixed(0).replaceAllMapped(new RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')} units',
                    style: CustomTextStyles(dataObject.context).holdingSubValueStyle),
                Text(
                    holding.change < 0
                        ? '-${dataObject.userCurrencySymbol}${(holding.change * -1).toStringAsFixed(2).replaceAllMapped(new RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')} (${(holding.changePercent * -1).toStringAsFixed(2).addCommas()}%)'
                        : '+${dataObject.userCurrencySymbol}${(holding.change).toStringAsFixed(2).replaceAllMapped(new RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')} (${holding.changePercent.toStringAsFixed(2).addCommas()}%)',
                    style: CustomTextStyles(dataObject.context).holdingSubValueStyle.copyWith(
                        fontWeight: FontWeight.w700, color: holding.change > 0 ? greenVarient : redVarient)),
              ],
            ),
          ),
        ],
