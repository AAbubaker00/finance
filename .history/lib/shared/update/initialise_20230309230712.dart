...
class Initialise {
  ...
  Map updatePortfolio(Map portfolio, {List filteredHoldings, rates}) {
    ...
    for (var holdingType in portfolio['assets']) {
      if (holdingType['id'] == 'stocks' && holdingType['items'].isNotEmpty) {
      ...
      } else if (holdingType['id'] == 'cryptos' && holdingType['items'].isNotEmpty) {
        holdingType['change'] = 0.0;
        holdingType['dailyChange'] = 0.0;
        holdingType['invested'] = 0.0;
        holdingType['view'] = true;

        // print(filteredHoldings);

        for (var holding in holdingType['items']) {
          if (filteredHoldings != null) {
            int holdingIndex = filteredHoldings.indexWhere(
                (element) => element['symbol'] == holding['symbol'] && element['typeId'] == 'cryptos');

            // print(filteredHoldings[holdingIndex]['marketData']);
            // print(holdingIndex);
          ...
          // print(cvrtRate);
          // PrintFunctions().printStartEndLine(holding['marketData']['regularMarketPrice']);

          // print('${holding['symbol']} : $cvrtRate');
          // print(holding['symbol']);

          holding['marketPrice'] =
              ((double.parse(holding['marketData']['regularMarketPrice'].toString()))) * cvrtRate;

          // holding['regularMarketChange'] =
          //     ((double.parse(holding['marketData']['regularMarketChange'].toString()))) * cvrtRate;

          // holding['dailyChange'] = ((holding['regularMarketChange'] * holding['shares'])) * cvrtRate;

          // PrintFunctions().printStartEndLine(  holding['dailyChange']);
        ...
        // portfolio['dailyChangePercent'] += (portfolio['dailyChange'] / portfolio['investedValue']) * 100;
        portfolio['returnPercentage'] = (portfolio['change'] / portfolio['investedValue']) * 100;
      } else if (holdingType['id'] == 'commodities' && holdingType['items'].isNotEmpty) {
        holdingType['change'] = 0.0;
        holdingType['dailyChange'] = 0.0;
        holdingType['invested'] = 0.0;
        holdingType['view'] = true;
        ...
        for (var holding in holdingType['items']) {
          // PrintFunctions().printStartEndLine(commodities['values']['commodities']);

          // PrintFunctions().printStartEndLine(holding);

          if (commodities != null) {
            int holdingIndex = commodities['values']['commodities']
                .indexWhere((element) => element['symbol'] == holding['symbol']);

            holding['marketData'] = commodities['values']['commodities'][holdingIndex];
          }
          ...
          holding['marketPrice'] = ((double.parse(holding['marketData']['rate'].toString()))) * cvrtRate;
