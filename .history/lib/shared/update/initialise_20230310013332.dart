    ...
    for (var holdingType in portfolio['assets']) {
      if (holdingType['id'] == 'stocks' && holdingType['items'].isNotEmpty) {
        ...
        for (var holding in holdingType['items']) {
          ...
          String currency = linkedQuote.currency.toString().capitalizeAll();

          double cvrtRate = getRate(rates, currency);

          holding['marketPrice'] =
              ((double.parse(linkedQuote.regularMarketPrice.toString()) / (isLSE ? 100.0 : 1.0))) * cvrtRate;
          ...
          if (linkedQuote.containsTrailingAnnualDividendRate) {
