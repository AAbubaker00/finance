...
class MarketUpdate {
  ...
  MarketUpdate(this.baseCurrency);

  filterHoldings(List portfolios){
    for (var portfolio in portfolios) {
      for (var asset in portfolio['assets']) {
        if (assetFilter.isEmpty) {
          assetFilter.add(asset);
        } else {
          int isAssetExist = assetFilter.indexWhere((element) => element['symbol'] == asset['symbol']);
  ...
  updateValues(List portfolios, BuildContext context,) {
    for (var portfolio in portfolios) {
      for (var asset in portfolio['assets']) {
        if (assetFilter.isEmpty) {
          assetFilter.add(asset);
    ...
    getRates();
    return getMarketData(portfolios, context);
  }

  getRates() async {
    newRates = await DatabaseService().getRates();
  }

  Future<List> getMarketData(List portfolios, BuildContext context) async {
    for (var holding in assetFilter) {
      holding['marketData'] =
          await YahooApiService().getAllAssetData(symbol: holding['symbol'], exchange: holding['exchange']);
      counter++;

      if (counter == assetFilter.length) {
        for (var portfolio in portfolios) {
          portfolio = updatePortfolio(portfolio, filterAssets: assetFilter, rates: newRates);
