...
class MarketUpdate {
  ...
  MarketUpdate({@required this.baseCurrency, this.data, this.context, this.databaseData});
  ...
  getMarketData() async {
    filteredHoldings.forEach((asset) async {
      ...
      if (counter == filteredHoldings.length) {
        for (var portfolio in portfolios) {
          portfolio = updatePortfolio(portfolio, filterAssets: filteredHoldings, rates: newRates);
