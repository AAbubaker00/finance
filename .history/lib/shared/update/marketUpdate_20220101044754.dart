...
class MarketUpdate {
  ...
  MarketUpdate(
      {@required this.baseCurrency, this.data, this.context, this.databaseData, this.documentSnapshot, this.states});
  ...
  getRates() async {
    if (main_Rates == null) {
      main_Rates = await DatabaseService().getRates();
    }
  }
  ...
  getMarketData() async {
    filteredHoldings.forEach((asset) async {
      ...
      if (counter == filteredHoldings.length) {
        for (var portfolio in portfolios) {
          portfolio = updatePortfolio(portfolio, filterAssets: filteredHoldings, rates: main_Rates);
        }
        ...
        Navigator.pushReplacement(
            ...
            MaterialPageRoute(
                ...
                settings: RouteSettings(arguments: {
                  ...
                  'news': null,
                  'filteredAssets': filteredHoldings,
                  'rates': main_Rates,
                })));
      }
    });
  ...
  Map updatePortfolio(Map portfolio, {List filterAssets, Map rates}) {
    ...
    for (var assetType in portfolio['assets']) {
      for (var asset in assetType['items']) {
        ...
        double cvrtRate = getRate(main_Rates, quote);
