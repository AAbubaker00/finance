  ...
  Map updatePortfolio(Map portfolio, {List filterAssets, Map rates}) {
    ...
    for (var assetType in portfolio['assets']) {
      for (var asset in assetType['items']) {
        ...
        asset['regularMarketChange'] = ((double.parse(asset['marketData']['quote']['regularMarketChange'].toString()) /
                (asset['exchange'] == 'LSE' ? 100.0 : 1.0))) *
            cvrtRate;
        ...
        asset['dailyChange'] =
            asset['regularMarketChange'] - (double.parse(asset['convrtBuyPrice'].toString()));
        asset['dailyPercDiff'] =
            (double.parse(asset['change'].toString()) / double.parse(asset['convrtBuyPrice'].toString())) *
                100;

        asset['value'] = asset['marketPrice'] * double.parse(asset['shares'].toString());
